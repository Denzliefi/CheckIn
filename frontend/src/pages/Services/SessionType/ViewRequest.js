...
function getExplicitId(raw) {
  return raw?.id || raw?.requestId || raw?.counselingRequestId || raw?._id || raw?.uuid || null;
}
...
function stableRequestId(raw) {
  ...
  const basis = JSON.stringify({
    type: raw?.type || (raw?.sessionType || raw?.date || raw?.time ? "MEET" : "ASK"),
    ...
    meetingLink:
      raw?.meetingLink || raw?.meetingUrl || raw?.onlineMeetingLink || raw?.meetLink || "",
  });
...
function normalizeRequest(raw) {
  ...
  const meetingLink = raw.meetingLink || raw.meetingUrl || raw.onlineMeetingLink || raw.meetLink || "";
          ...
          <div className="cc-hero-inner">
            ...
            <div className="mt-3">
              ...
              <p className="cc-subtitle">
                Track your sessions and questions with a clean, friendly timeline.
              </p>
            </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <div className="text-xs sm:text-sm text-slate-600">
                Tip: Use search to quickly find a request by topic, counselor, or date.
              </div>
            </div>
          </div>
      ...
      <div className="cc-list-sticky">
        <div className="rounded-2xl bg-white/85 border border-gray-200 p-3.5 shadow-sm backdrop-blur">
          ...
          <div className="mt-3 text-sm text-gray-500 flex items-center justify-between gap-3">
            ...
            <div className="hidden sm:block text-xs text-gray-400">Tap a request to open details</div>
          </div>
        </div>
      </div>
        ...
        x === "…" ? (
          ...
          <button
            ...
            className={`cc-focus cc-clickable cc-page-num ${x === page ? "cc-page-num-active" : ""} ${
              animDir === "in-right" ? "cc-fade-right" : animDir === "in-left" ? "cc-fade-left" : ""
            }`}
        ...
        <Section title="Info">
          ...
          {isMeet ? (
            ...
            <>
              ...
              <KeyValue
                label="Counselor"
                value={item.counselorReply ? item.counselorName || "Assigned" : "Assigned when replied"}
              />
            </>
          )}
...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-gray-200 bg-white/90 p-10 text-center cc-fade-in shadow-sm backdrop-blur">
      ...
      <div className="text-sm text-gray-500 mt-2">Use the button above to create one.</div>
    </div>
  );
}
...
function NoResults({ search, onClear }) {
  return (
    <div className="rounded-2xl border border-gray-200 bg-white/90 p-10 text-center cc-fade-in shadow-sm backdrop-blur">
      ...
      <div className="text-sm text-gray-500 mt-2">Nothing matched “{search.trim()}”.</div>
      <button
        type="button"
        onClick={onClear}
        ...
        >
          ...
          <div className="px-3 sm:px-5 pb-4 border-b border-gray-200 flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-sm text-gray-500">Swipe down or tap outside to close</div>
            </div>
