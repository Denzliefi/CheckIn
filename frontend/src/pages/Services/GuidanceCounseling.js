...
function BackgroundFX() {
  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      <div className="absolute -top-40 -left-40 h-[560px] w-[560px] rounded-full blur-3xl"
        style={{ background: `radial-gradient(circle at 30% 30%, rgba(185,255,102,0.95), transparent)`, opacity: 0.38 }} />
      <div className="absolute -top-44 -right-44 h-[680px] w-[680px] rounded-full blur-3xl"
        style={{ background: `radial-gradient(circle at 30% 30%, rgba(218,252,182,0.95), transparent)`, opacity: 0.30 }} />
      <div className="absolute top-[18%] left-[18%] h-[720px] w-[720px] rounded-full blur-3xl"
        style={{ background: `radial-gradient(circle at 30% 30%, rgba(211,243,176,0.85), transparent)`, opacity: 0.22 }} />
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-16 sm:py-20 md:py-24 lg:py-28"> {/* ← reduced vertical padding, adjust as needed */}
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10 lg:gap-14 items-center">
            ...
            <div className="md:col-span-6 lg:col-span-5 space-y-6 md:space-y-8">
              <FadeUp>
                <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/78 backdrop-blur border border-black/10 shadow-[0_10px_22px_rgba(0,0,0,0.04)] font-extrabold text-black/70 text-xs sm:text-sm">
                  <span className="size-2.5 rounded-full" style={{ backgroundColor: CHECKIN_GREEN, boxShadow: "0 0 0 3px rgba(185,255,102,0.18)" }} />
                  Confidential • 24/7 Support
                </div>
              </FadeUp>

              <FadeUp delay={80}>
                <h1 className={`font-black text-[#141414] ${TXT.xl}`} style={{ fontFamily: "Lora, serif" }}>
                  Talk to someone who{" "}
                  <span className="relative inline-block">
                    really gets it
                    <span className="absolute -bottom-2 left-0 right-0 h-2.5 sm:h-3 -z-10 rounded-full" style={{ background: "rgba(185,255,102,0.55)" }} />
                  </span>
                </h1>
              </FadeUp>
                ...
                <FadeUp delay={240}>
                  <div className="flex flex-wrap gap-3 pt-2">
                    {["Student-First", "Easy Scheduling", "100% Confidential"].map((label) => (
                      <span key={label} className="px-4 py-1.5 rounded-full bg-white/78 backdrop-blur border border-black/10 shadow-[0_10px_26px_rgba(0,0,0,0.04)] text-black/70 font-extrabold text-xs sm:text-sm">
                        {label}
                      </span>
                    ))}
                  </div>
              ...
              <FadeUp delay={340}>
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <button
                    ...
                    style={{ backgroundColor: CHECKIN_GREEN, color: CHECKIN_DARK, boxShadow: "0 18px 50px rgba(185,255,102,0.45)" }}
                  >
                    Request a Session →
                  </button>
            ...
            {/* RIGHT - image side */}
            <FadeUp delay={120} className="md:col-span-6 lg:col-span-7 flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[380px] sm:max-w-[460px] md:max-w-[420px] lg:max-w-[580px]">
                <div className="absolute inset-0 rounded-[34px] blur-3xl -rotate-2 scale-105"
                  style={{ background: "radial-gradient(720px 360px at 20% 10%, rgba(185,255,102,0.45), transparent 65%)" }} />
                ...
                <GlassCard className="p-3 sm:p-4">
                  <div className="relative rounded-[22px] overflow-hidden border border-black/10 bg-white">
                    <div className="block sm:hidden aspect-[4/3]">
                      <img src={friendImg} alt="Friendly counseling session" className="w-full h-full object-cover" draggable={false} />
                    </div>
                    <img src={friendImg} alt="Friendly counseling session" className="hidden sm:block w-full h-auto object-cover" draggable={false} />
                    <div className="pointer-events-none absolute inset-0" style={{ boxShadow: "inset 0 -120px 120px rgba(0,0,0,0.10)" }} />
                  </div>
                </GlassCard>
